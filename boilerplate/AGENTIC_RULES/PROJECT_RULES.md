...
Renseigner toutes les occurrences des marqueurs entre doubles accolades avant
utilisation, y compris dans les exemples d'appels. Ne jamais appeler un service
avec un marqueur, une valeur vide ou un identifiant deviné. La copie de ces règles
ne configure aucun serveur MCP et ne crée aucun espace mémoire.

| Usage | Serveur MCP | Identifiant |
| --- | --- | --- |
| Mémoire de travail | `{{LIVE_MCP_SERVER}}` | `space_id="{{SPACE_ID}}"` |
| Index sémantique durable | `{{GRAPH_MCP_SERVER}}` | `memory_id="{{GRAPH_MEMORY_ID}}"` |
...
Graph Memory peut être retiré si le projet n'utilise pas cet index : retirer
alors sa ligne de configuration et sa section de procédure. Live Memory et son
...
2. Lire `space_rules(space_id="{{SPACE_ID}}")` une fois pour connaître la structure.
...
4. Lire les notes récentes avec `live_read(space_id="{{SPACE_ID}}")`. Si le résultat est
...
`insight`, `question`. Toujours renseigner `space_id="{{SPACE_ID}}"`.
...
Appeler `bank_consolidate(space_id="{{SPACE_ID}}")` une seule fois, puis suivre la
...
Cibler le serveur `{{GRAPH_MCP_SERVER}}` avec l'identifiant
`memory_id="{{GRAPH_MEMORY_ID}}"`, jamais un outil homonyme d'un autre serveur.
