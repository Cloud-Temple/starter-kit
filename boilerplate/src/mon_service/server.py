...
async def system_whoami() -> dict:
    ...
    Retourne le nom du client, les permissions, les ressources autorisées,
    et les métadonnées du token (email, dates de création/expiration).
    ...
    # Métadonnées du token S3 (si ce n'est pas le bootstrap key)
    if info.get("email"):
...
def create_app():
    ...
    bloquées par l'auth ou les health checks n'apparaissent pas dans les logs.
